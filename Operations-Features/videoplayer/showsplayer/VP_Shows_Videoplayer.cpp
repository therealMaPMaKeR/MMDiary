...
void VP_Shows_Videoplayer::connectSignals()
{
    ...
    connect(m_stopButton, &QPushButton::clicked,
            this, &VP_Shows_Videoplayer::close);
...
void VP_Shows_Videoplayer::closeEvent(QCloseEvent *event)
{
    ...
    // Exit fullscreen if active (but skip the complex window restoration since we're closing anyway)
    if (m_isFullScreen) {
        qDebug() << "VP_Shows_Videoplayer: Skipping exitFullScreen() during close to prevent signal interference";
        // Just stop the timers and restore cursor without window manipulation
        stopCursorTimer();
        showCursor();
        m_isFullScreen = false;
    }
