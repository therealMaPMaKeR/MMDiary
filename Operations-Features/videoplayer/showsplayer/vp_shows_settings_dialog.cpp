...
VP_ShowsSettingsDialog::VP_ShowsSettingsDialog(QWidget *parent)
    : QDialog(parent)
{
    setupUi();
    loadSettings();
...
void VP_ShowsSettingsDialog::setupUi()
{
    setWindowTitle("TV Shows Settings");
    setMinimumWidth(500);
    ...
    // Connect signals
    connect(m_enableTMDBCheckBox, &QCheckBox::toggled, this, &VP_ShowsSettingsDialog::onTMDBEnabledToggled);
    connect(m_testButton, &QPushButton::clicked, this, &VP_ShowsSettingsDialog::onTestConnectionClicked);
    connect(m_saveButton, &QPushButton::clicked, this, &VP_ShowsSettingsDialog::onSaveClicked);
...
void VP_ShowsSettingsDialog::loadSettings()
{
    ...
    qDebug() << "VP_ShowsSettingsDialog: Settings loaded";
}

void VP_ShowsSettingsDialog::onSaveClicked()
{
    ...
    if (tmdbEnabled) {
        ...
        if (!apiKey.isEmpty()) {
            ...
            qDebug() << "VP_ShowsSettingsDialog: API key saved";
    ...
    QMessageBox::information(this, "Settings Saved", "TV Shows settings have been saved.");
    accept();
}

void VP_ShowsSettingsDialog::onTestConnectionClicked()
{
    QString apiKey = m_apiKeyEdit->text().trimmed();
    ...
    if (success && testShow.tmdbId > 0) {
        ...
        qDebug() << "VP_ShowsSettingsDialog: TMDB connection test successful";
    } else {
        QMessageBox::warning(this, "Connection Failed",
                           "Failed to connect to TMDB. Please check your API key and internet connection.");
        qDebug() << "VP_ShowsSettingsDialog: TMDB connection test failed";
    }
}

void VP_ShowsSettingsDialog::onTMDBEnabledToggled(bool checked)
{
    ...
    qDebug() << "VP_ShowsSettingsDialog: TMDB enabled toggled:" << checked;
}
