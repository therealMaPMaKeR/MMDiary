...
bool DatabaseSettingsManager::IndexIsValid(QString index, QString type)
{
    ...
    if (columnTypes.isEmpty()) {
        ...
        // Misc Settings columns
        columnTypes[Constants::SettingsT_Index_Misc_DecryptToRAM] = Constants::DataType_QString;
        columnTypes[Constants::SettingsT_Index_Misc_MinFreeRAM] = Constants::DataType_QString;
    }
...
bool DatabaseSettingsManager::migrateToV5()
{
    // Migration to version 5: Add Misc settings columns
    qDebug() << "========================================";
    qDebug() << "DatabaseSettingsManager: Starting migration to v5";
    qDebug() << "========================================";
    ...
    // NEW Misc Settings (v5)
    qDebug() << "DatabaseSettingsManager: Adding Misc settings columns to schema...";
    settingsTableColumns[Constants::SettingsT_Index_Misc_DecryptToRAM] = "TEXT";
    settingsTableColumns[Constants::SettingsT_Index_Misc_MinFreeRAM] = "TEXT";
    qDebug() << "DatabaseSettingsManager: Total columns in new schema:" << settingsTableColumns.size();

    // Create temporary table with new schema
    qDebug() << "DatabaseSettingsManager: Creating temporary table with v5 schema...";
    if (!m_dbManager.createTable("settings_temp", settingsTableColumns)) {
        qCritical() << "DatabaseSettingsManager: FAILED to create temporary settings table for v5 migration:" << m_dbManager.lastError();
        return false;
    }
    qDebug() << "DatabaseSettingsManager: Temporary table created successfully";

    // Build column list for data migration (only columns that exist in the old v4 table)
    QStringList existingColumns;
    existingColumns << "id"
                    << Constants::SettingsT_Index_Displayname
                    << Constants::SettingsT_Index_DisplaynameColor
                    ...
                    << Constants::SettingsT_Index_DataENC_Hide_Categories
                    << Constants::SettingsT_Index_DataENC_Hide_Tags
                    << Constants::SettingsT_Index_VP_Shows_Autoplay
                    << Constants::SettingsT_Index_VP_Shows_AutoFullScreen
                    << Constants::SettingsT_Index_VP_Shows_UseTMDB
                    << Constants::SettingsT_Index_VP_Shows_DisplayFilenames
                    << Constants::SettingsT_Index_VP_Shows_CheckNewEP
                    << Constants::SettingsT_Index_VP_Shows_FileFolderParsing
                    << Constants::SettingsT_Index_VP_Shows_AutoDelete
                    << Constants::SettingsT_Index_VP_Shows_DefaultVolume
                    << Constants::SettingsT_Index_VP_Shows_CheckNewEPStartup;
    ...
    qDebug() << "DatabaseSettingsManager: Executing copy query...";
    if (!m_dbManager.executeQuery(copyQuery)) {
        qCritical() << "DatabaseSettingsManager: FAILED to copy data to temporary table for v5 migration:" << m_dbManager.lastError();
        m_dbManager.dropTable("settings_temp");
        return false;
    }
    qDebug() << "DatabaseSettingsManager: Data copied successfully";
    ...
    qDebug() << "DatabaseSettingsManager: Dropping old settings table...";
    if (!m_dbManager.dropTable("settings")) {
        qCritical() << "DatabaseSettingsManager: FAILED to drop old settings table for v5 migration:" << m_dbManager.lastError();
        m_dbManager.dropTable("settings_temp");
        return false;
    }
    qDebug() << "DatabaseSettingsManager: Old table dropped successfully";
    ...
    qDebug() << "DatabaseSettingsManager: Renaming temporary table to settings...";
    if (!m_dbManager.executeQuery(renameQuery)) {
        qCritical() << "DatabaseSettingsManager: FAILED to rename temporary table for v5 migration:" << m_dbManager.lastError();
        return false;
    }
    qDebug() << "DatabaseSettingsManager: Table renamed successfully";

    // Now set default values for the new Misc settings columns
    qDebug() << "DatabaseSettingsManager: Setting default values for Misc settings...";
    
    // Ensure settings record exists
    if (!ensureSettingsRecord()) {
        qCritical() << "DatabaseSettingsManager: FAILED to ensure settings record exists for v5 migration";
        return false;
    }
    
    // Set default values for Misc settings directly within the migration transaction
    bool success = true;
    success &= UpdateSettingsData_TEXT(Constants::SettingsT_Index_Misc_DecryptToRAM, 
                                       Default_UserSettings::DEFAULT_MISC_DECRYPT_TO_RAM);
    success &= UpdateSettingsData_TEXT(Constants::SettingsT_Index_Misc_MinFreeRAM,
                                       Default_UserSettings::DEFAULT_MISC_MIN_FREE_RAM);
    
    if (!success) {
        qWarning() << "DatabaseSettingsManager: Some Misc default values may not have been set during v5 migration";
        // Don't fail the migration - the columns exist and can be populated later
    } else {
        qDebug() << "DatabaseSettingsManager: All Misc default values set successfully";
    }

    qDebug() << "========================================";
    qDebug() << "DatabaseSettingsManager: Migration to v5 COMPLETED SUCCESSFULLY";
    qDebug() << "========================================";
    return true;
}

bool DatabaseSettingsManager::rollbackFromV5()
{
    // Rollback from version 5: Remove Misc settings columns
    qDebug() << "DatabaseSettingsManager: Rolling back from v5 - removing Misc settings";
    
    // Create table with v4 schema (without Misc settings)
    QMap<QString, QString> settingsTableColumns;
    settingsTableColumns["id"] = "INTEGER PRIMARY KEY";
    ...
    // VideoPlayer Settings (keep from v4)
    settingsTableColumns[Constants::SettingsT_Index_VP_Shows_Autoplay] = "TEXT";
    settingsTableColumns[Constants::SettingsT_Index_VP_Shows_AutoFullScreen] = "TEXT";
    settingsTableColumns[Constants::SettingsT_Index_VP_Shows_UseTMDB] = "TEXT";
    settingsTableColumns[Constants::SettingsT_Index_VP_Shows_DisplayFilenames] = "TEXT";
    ...
    // Create temporary table with v4 schema
    if (!m_dbManager.createTable("settings_temp", settingsTableColumns)) {
        qWarning() << "Failed to create temporary settings table for v5 rollback:" << m_dbManager.lastError();
        return false;
    }

    // Build column list for data migration (only columns that exist in v4)
    QStringList v4Columns;
    v4Columns << "id"
              << Constants::SettingsT_Index_Displayname
              << Constants::SettingsT_Index_DisplaynameColor
              << Constants::SettingsT_Index_MinToTray
    ...
    QString columnList = v4Columns.join(", ");

    // Copy data from current table to temporary table (excluding Misc columns)
    QString copyQuery = QString("INSERT INTO settings_temp (%1) SELECT %1 FROM settings").arg(columnList);
    if (!m_dbManager.executeQuery(copyQuery)) {
        qWarning() << "Failed to copy data for v5 rollback:" << m_dbManager.lastError();
        m_dbManager.dropTable("settings_temp");
        return false;
    }
    ...
    // Drop current table
    if (!m_dbManager.dropTable("settings")) {
        qWarning() << "Failed to drop settings table during v5 rollback:" << m_dbManager.lastError();
        m_dbManager.dropTable("settings_temp");
        return false;
    }
    ...
    QString renameQuery = "ALTER TABLE settings_temp RENAME TO settings";
    if (!m_dbManager.executeQuery(renameQuery)) {
        qWarning() << "Failed to rename temporary table during v5 rollback:" << m_dbManager.lastError();
        return false;
    }

    qDebug() << "Successfully rolled back settings database from version 5";
    return true;
}
